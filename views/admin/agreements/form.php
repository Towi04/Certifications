...
$tab = $tab ?? (string) ($_GET['tab'] ?? 'version');
$tabs = ['version' => 'Versión'];
if ($item) {
    $tabs['publicar'] = 'Publicar';
    $tabs['firmas'] = 'Firmas';
...
$fichaTitle = $item ? (string) ($item['name'] ?? 'Convenio') : 'Nueva versión de convenio';
$fichaSubtitle = $item
    ? e((string) ($item['tier_name'] ?? 'Nivel TR')) . ' · ' . (int) ($item['year'] ?? 0)
        . (!empty($item['is_current']) ? ' · <span class="pill">Vigente</span>' : '')
    : 'Sube la plantilla PDF y publícala a todos los partners del nivel.';
$fichaBackUrl = '/admin/agreements';
$fichaTabBase = $item ? '/admin/agreements/edit?id=' . (int) $item['id'] : '';
$fichaMode = 'js';
$fichaInitial = 'C';
?>
<section class="admin-ficha" data-admin-ficha data-tab="<?= e($tab) ?>">
    ...
    <form method="post" action="/admin/agreements/save" class="stack" enctype="multipart/form-data">
    ...
    <?php if ($item): ?>
        <div class="admin-ficha-panel" data-tab-panel="publicar" <?= $tab !== 'publicar' ? 'hidden' : '' ?>>
        ...
        <div class="admin-ficha-panel" data-tab-panel="firmas" <?= $tab !== 'firmas' ? 'hidden' : '' ?>>
